...
class Simple_Local_Avatars {
    ...
    public function get_avatar( $avatar = '', $id_or_email = '', $size = 96, $default = '', $alt = '' ) {
        ...
        $avatar = "<img alt='" . esc_attr( $alt ) . "' src='" . $local_avatars[$size] . "' class='avatar avatar {$author_class} photo' height='{$size}' width='{$size}' />";
    ...
    public function admin_enqueue_scripts( $hook_suffix ) {
        ...
        wp_enqueue_script( 'simple-local-avatars', WPSTORM_JS_PATH . 'simple-local-avatars' . $script_name_append . '.js', array('jquery'), false, true );
...
if ( ! function_exists( 'get_avatar' ) && ( $simple_local_avatars_options = get_option('simple_local_avatars') ) && ! empty( $simple_local_avatars_options['only'] ) ) :
    ...
    function get_avatar( $id_or_email, $size = 96, $default = '', $alt = '' ) {
        ...
        if ( ! $avatar = $simple_local_avatars->get_avatar( '', $id_or_email, $size, $default, $alt ) ) :
            ...
            $avatar = "<img alt='{$safe_alt}' src='" . $default . "' class='avatar avatar photo avatar-default' height='{$size}' width='{$size}' />";
